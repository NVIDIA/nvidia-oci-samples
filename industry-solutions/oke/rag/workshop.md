...
> **Note**: This workshop uses the Text Only configuration (4 GPUs on H100, 5 GPUs on A100), which supports text extraction from PDFs. Use **1 node** — Text Only fits on a single 8-GPU node. The Full Multimodal configuration adds support for tables, charts, and images; see the [RAG Blueprint on OKE Guide](README.md) for node count and options.
...
- [RAG Blueprint on OKE Guide](README.md) — Full Multimodal, node count, CLI deployment
