...
# NODE_POOL_NAME (default nemotron-lightning-a10x2), BOOT_VOLUME_GB (default 250), OCI_CLI_PROFILE, OCI_CLI_AUTH.
set -euo pipefail
...
POOL_ID=$("${OCI[@]}" ce node-pool create \
  ...
  --initial-node-labels '[{"key": "app", "value": "nemotron-lightning"}]' \
...
echo "waiting for the node to become ACTIVE (about 10 minutes)..."
...
echo "waiting for the node to be Ready in Kubernetes with its GPUs and full disk..."
until kubectl get nodes -l node.kubernetes.io/instance-type=VM.GPU.A10.2 -o jsonpath='{.items[0].status.capacity.nvidia\.com/gpu}' 2>/dev/null | grep -q '^2$'; do sleep 15; done
kubectl get nodes -l node.kubernetes.io/instance-type=VM.GPU.A10.2 \
  -o custom-columns='NODE:.metadata.name,READY:.status.conditions[?(@.type=="Ready")].status,GPUS:.status.capacity.nvidia\.com/gpu,EPHEMERAL:.status.capacity.ephemeral-storage'
