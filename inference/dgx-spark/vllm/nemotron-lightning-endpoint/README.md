...
cd <clone>/inference/dgx-spark/vllm/nemotron-lightning-endpoint
...
> upstream repository first, and use the fork only if the `inference/dgx-spark/` directory isn't
> there yet.

Run the sample:

```bash
cd nvidia-oci-samples/inference/dgx-spark/vllm/nemotron-lightning-endpoint
...
git add inference/dgx-spark/vllm/nemotron-lightning-endpoint README.md
